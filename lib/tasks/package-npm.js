...
gulp.task('package:npm:prepare', () => {
  ...
  // Prepend component macro import in layouts, so it's always loaded
  gulp.src(paths.bundleTemplates + '**/*.njk')
    .pipe(insert.prepend('{% import "components.njk" as govuk %}\n\n'))
    .pipe(gulp.dest(paths.npmTemplates))

...
customTheme.on('init', function (env, app) {
  env.engine.addFilter('generateComponentInvocation', (args, format, componentName) => {
    ...
    for (var key in args) {
      if (args.hasOwnProperty(key)) {
        argFormatted = `${key}${separator}${JSON.stringify(args[key], null, 2)}`
        if (format === 'erb') {
          argFormatted = argFormatted.replace(/": "/g, '" => "')
        }
        argsFormatted.push(argFormatted.replace(/\n/g, '\n    '))
      }
    }
